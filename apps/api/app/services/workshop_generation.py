...
9. Do not rename, rebrand, or coin source-sounding names for derived methods, frameworks, rules, designs, strategies, processes, models, or approaches. Preserve supplied concept names when referring to source knowledge. Do not turn a generic source idea into a new Title Case label such as "Variation Rule", "Interleaved Design", or "Buffer-Responsive Design". If a temporary label genuinely improves usability, explicitly say in the same sentence that it is a label created for this draft and list it in design_choices.
